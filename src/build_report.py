...
def build_report(data_path, output_path):
    ...
    # FIX: found via investigating an independent review's claim that
    # "Uncertain" outcomes get counted toward the headline completeness
    # percentage. Directly tracing igReadiness's own calculation found
    # it partly depends on Tier 2's own self-reported
    # audit_completion_percent (an LLM-generated value, not something
    # deterministically auditable from this code alone) — so rather
    # than claim to have fully resolved that specific ambiguity, this
    # adds a second, explicit, wholly deterministic figure computed
    # only from the counts immediately above: genuinely-concluded
    # procedures (Passed + Raised Issue) as a share of all planned
    # procedures, with Uncertain and Not Run excluded by construction,
    # not by assumption.
    cov_total_planned = cov_pass + cov_issue + cov_unc + cov_np
    cov_conclusive_pct = round(((cov_pass + cov_issue) / cov_total_planned) * 100) if cov_total_planned > 0 else 0
    ...
    if p1_open > 0 or critical_query_open > 0 or other_gates_open:
        ...
        if igReadiness >= 60:
            ...
            verdict_text=(f'{_blocker_desc} remain open — this model cannot be classified reliance-ready for management, lender or investor use until these are resolved (or, for a Critical Query, confirmed either way) and retested. '
                          f'{igReadiness}% of planned procedures completed ({cov_pass} passed, {cov_issue} raised issues, {cov_unc} uncertain, {cov_np} not run) — {cov_conclusive_pct}% concluded (passed or raised issue) once Uncertain and Not Run are excluded.')
        else:
            verdict_short='NOT RELIANCE-READY'; verdict_bg=DARK_BLUE
            verdict_text=(f'{_blocker_desc} open and {igReadiness}% of planned procedures completed ({cov_conclusive_pct}% concluded, Uncertain/Not Run excluded). Both open reliance blockers and audit coverage prevent reliance at any level. '
