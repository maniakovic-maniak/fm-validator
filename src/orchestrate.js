...
const { PHASE_A_OUTPUT_SCHEMA, PHASE_B_OUTPUT_SCHEMA } = require('./finding-schema');
 ...
 */
// Shared across every phase's API call, deliberately - all phases share
// the same Constitution+Soul+Skill prefix (Phase C additionally appends
// the Playbook after it), so using one consistent key improves real
// cache-hit routing across the many separate calls a single engagement
// makes, rather than relying purely on OpenAI's own content-hash
// matching.
const PARTNER_REVIEW_CACHE_KEY = 'partner-review-base-prompt-v1';
...
async function runPhaseA(engagementId, modelDataBatches) {
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    const response = await client.responses.create({
      ...
      prompt_cache_key: PARTNER_REVIEW_CACHE_KEY,
    });
...
async function runPhaseB(frozenPhaseAPath, fmValidatorReportData, visibilityRecord = null) {
  ...
  const response = await client.responses.create({
    ...
    tools: [{ type: 'web_search_preview' }],
    text: { format: PHASE_B_OUTPUT_SCHEMA },
    prompt_cache_key: PARTNER_REVIEW_CACHE_KEY,
  });
...
/**
 * Saves Phase B's result scoped to its own engagement - fixes a real bug
 * where the original test script wrote to one single, hardcoded path
 * regardless of engagement, meaning a second, separate engagement's
 * Phase B run would silently overwrite the first's result entirely.
 */
function savePhaseBResult(engagementId, result) {
  const resultPath = path.join(ENGAGEMENTS_DIR, `${engagementId}-phase-b-result.json`);
  fs.writeFileSync(resultPath, JSON.stringify(result, null, 2));
  return resultPath;
}

module.exports = { runPhaseA, freezePhaseA, runPhaseB, savePhaseBResult, ENGAGEMENTS_DIR };
