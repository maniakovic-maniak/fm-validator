...
(async () => {
  const files = [
    ['/mnt/user-data/uploads/The_Bend_FInancial_Model_3_8_2026.xlsx', 1, 4],
  ...
  for (const [file, expectCount, expectInstances] of files) {
    ...
    if (expectInstances !== null) {
      check(`end-to-end: ${file.split('/').pop()} — instanceCount === ${expectInstances}`, result.findings[0].instanceCount === expectInstances);
    }
  }
