...
async function run() {
  ...
  // FIX (R-4): found via the same independent review — a balance-sheet
  // equity COMPONENT (contributed equity, paid-in capital) computed as
  // a residual of Total Assets/Liabilities rather than as an
  // independent figure. Distinct from the existing labeled-plug
  // check; this catches an unlabeled one via formula structure.
  ...
  // FIX (R-5): found via the same independent review — a formula
  // template that changes cleanly at a single column mid-row (a
  // cumulative-balance reference switching to a period-flow
  // reference). The existing majority-vote consistency check was
  // directly confirmed to miss this (a 66.7%/33.3% split, just under
  // its 70% threshold). Complementary detection strategy, not a
  // replacement — moderate confidence, since testing on another real
  // file confirmed a clean transition CAN also be a genuine,
  ...
  // FIX (I-1/I-2): found via an independent review confirming zero P1
  // findings and 226/226 "Investment Blocker: No" despite the model's
  // own named master controls showing FAIL. Surfaces these directly
  // as dedicated, high-severity, blocker-flagged findings rather than
  // depending on Tier 2 to notice and correctly prioritize them.
  ...
  // FIX (I-5): mathematically impossible COUNTA-sum control targets.
  ...
  // FIX (I-4/I-5): a fixed direct-link target compared against a
  // differently-computed (division/AVERAGE) actual for exact equality.
  ...
  // FIX (I-6): a release/status gate formula whose own tested range
  // doesn't cover all other status-like cells on the same sheet.
  ...
  // FIX (I-9): a guide/navigational sheet references a sheet by name
  // (SheetName!Cell text) that doesn't exist in the actual workbook.
  ...
  // FIX (I-10): reconciles this audit's own formula-cell count against
  // the model's own self-reported count, where one exists.
  ...
  if (recalcCheckResult.status === 'unavailable') {
    ...
    // ── Equity component backward-solved (R-4) — a contributed-equity
    // style line computed as a residual of Total Assets/Liabilities. ──
    ...
    // ── Mid-row formula regime change (R-5) — a clean formula-shape
    // break at a single column. Medium severity, moderate confidence:
    // confirmed directly this can also be a genuine, legitimate
    // actuals-to-forecast or phase boundary, not always a defect. ──
    ...
    // ── Master control failure (I-1/I-2) — the model's own named
    // control/gate cells show FAIL. Severity 'critical' and
    // investment_grade_blocker: true, confirmed against build_report.py's
    // own priority()/Investment Blocker logic, so these reach P1 and
    // are marked as blockers directly, not left dependent on Tier 2. ──
    ...
    // ── Impossible COUNTA control target (I-5, MR-004's pattern) —
    // a control formula tests a COUNTA-sum against a target that
    // mathematically exceeds the referenced ranges' max possible size. ──
    ...
    // ── Mismatched-basis comparison (I-4/I-5, ER-006/ER-007's pattern)
    // — a fixed direct-link target compared against a differently-
    // computed actual for exact equality. ──
    ...
    // ── Release gate coverage (I-6, MR-003's pattern) — a release/
    // status gate formula whose own tested range doesn't cover all
    // other status-like cells on the same sheet. ──
    ...
    // ── Formula count reconciliation (I-10, MR-011's pattern) — the
    // model's own self-reported formula count differs materially from
    // this audit's own formula-cell scan. ──
  ...
  // Classify every finding's record_type (Confirmed Finding / Query /
  // Critical Query / Observation / Scope Limitation / Not Applicable /
  // False Positive) before the report is built. Respects an explicit
  // record_type if a check (or a future Tier 2 run using the updated
  // soul.md) already set one; otherwise infers a default from existing
  // confidence/severity/urgency fields. Only record_type === 'Confirmed
